...
Insert with `ON CONFLICT (transaction_id) DO NOTHING`, then compare account, amount,
...
storage failures. A pinned Kafka 4.1.1 + PostgreSQL 17.6 integration test checks
persistence, spoofed type headers, identical and conflicting duplicates, and a SQL
constraint failure that leaves the offset unchanged before successful restart.
...
are not directly exercised yet. Supported-currency rules and durable rejection
events are outside this change.
